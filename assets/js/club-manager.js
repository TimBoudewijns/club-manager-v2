...
// Note: Since WordPress doesn't support ES6 modules by default, 
// we'll use a single file or load modules differently
...
document.addEventListener('alpine:init', () => {
    Alpine.data('clubManager', () => ({
        ...
        async createPlayer() {
            try {
                await this.apiPost('cm_create_player', {
                    ...this.newPlayer,
                    team_id: this.selectedTeam.id,
                    season: this.currentSeason
                });
        ...
        async addExistingPlayerToTeam() {
            try {
                await this.apiPost('cm_add_player_to_team', {
                    team_id: this.selectedTeam.id,
