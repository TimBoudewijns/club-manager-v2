...
(function() {
    ...
    function initClubManager() {
        if (typeof Alpine === 'undefined') {
            setTimeout(initClubManager, 100);
            return;
        }
        
        Alpine.data('clubManager', function() {
            return {
                // Core data
                activeTab: 'my-teams',
                ...
                init() {
                    ...
                    this.$watch('showCreateTeamModal', function(value) {
                        document.body.style.overflow = value ? 'hidden' : '';
                    });
                    this.$watch('showAddPlayerModal', function(value) {
                        document.body.style.overflow = value ? 'hidden' : '';
                    });
                    this.$watch('showAddExistingPlayerModal', function(value) {
                        document.body.style.overflow = value ? 'hidden' : '';
                    });
                    this.$watch('showEvaluationModal', function(value) {
                        document.body.style.overflow = value ? 'hidden' : '';
                    });
                    this.$watch('showPlayerHistoryModal', function(value) {
                        document.body.style.overflow = value ? 'hidden' : '';
                    });
                },
                ...
                handleRemoveClick(playerId) {
                    const player = this.teamPlayers.find(function(p) { return p.id == playerId; });
                    if (player) {
                ...
                initializeCurrentEvaluationScores() {
                    const self = this;
                    this.evaluationCategories.forEach(function(category) {
                        category.subcategories.forEach(function(sub) {
                            const key = category.key + '_' + sub.key;
                            self.currentEvaluationScores[key] = self.getLastSubcategoryScore(category.key, sub.key);
                        });
                    });
                },
                ...
                getLastSubcategoryScore(categoryKey, subcategoryKey) {
                    ...
                    const evaluations = this.evaluations[this.evaluatingPlayer.id]
                        .filter(function(e) { return e.category === categoryKey && e.subcategory === subcategoryKey; })
                        .sort(function(a, b) { return new Date(b.evaluated_at) - new Date(a.evaluated_at); });
                ...
                getCategoryAverage(categoryKey) {
                    const category = this.evaluationCategories.find(function(c) { return c.key === categoryKey; });
                    if (!category) return '5.0';
                    
                    const self = this;
                    let scores = [];
                    category.subcategories.forEach(function(sub) {
                        const score = self.getSubcategoryScore(categoryKey, sub.key);
                        scores.push(score);
                    });
                    
                    if (scores.length === 0) return '5.0';
                    
                    const average = scores.reduce(function(a, b) { return a + b; }, 0) / scores.length;
                ...
                async saveEvaluation() {
                    ...
                    try {
                        ...
                        if (this.viewingPlayer && this.viewingPlayer.id === this.evaluatingPlayer.id) {
                            ...
                            setTimeout(function() {
                                self.pollForAdvice(self.viewingPlayer);
                            }, 2000);
                        }
                ...
                async loadEvaluationHistory(player) {
                    try {
                        ...
                        this.playerEvaluationHistory = allEvaluations.filter(function(e) { return !e.subcategory; });
                        ...
                        this.availableEvaluationDates = dates.sort(function(a, b) { return new Date(b) - new Date(a); });
                        
                        // Sort by date descending
                        this.playerEvaluationHistory.sort(function(a, b) { return new Date(b.evaluated_at) - new Date(a.evaluated_at); });
                ...
                getFilteredEvaluationHistory() {
                    ...
                    return this.playerEvaluationHistory.filter(function(e) {
                        return e.evaluated_at.startsWith(selectedDate);
                    });
                },
                ...
                getSubcategoryEvaluations(category, evaluatedAt) {
                    ...
                    return this.evaluations[this.viewingPlayer.id].filter(function(e) {
                ...
                getPlayerCardCategoryAverage(categoryKey) {
                    ...
                    if (this.selectedEvaluationDate !== 'all') {
                        const selectedDate = this.selectedEvaluationDate;
                        evaluationsToUse = evaluationsToUse.filter(function(e) {
                            return e.evaluated_at.startsWith(selectedDate);
                        });
                    }
                    
                    // Try to get main category evaluations first
                    const categoryEvaluations = evaluationsToUse.filter(function(e) {
                        return e.category === categoryKey && !e.subcategory;
                    });
                    
                    if (categoryEvaluations.length > 0) {
                        const sum = categoryEvaluations.reduce(function(acc, eval) { return acc + parseFloat(eval.score); }, 0);
                    ...
                    const self = this;
                    category.subcategories.forEach(function(sub) {
                        const subEvaluations = evaluationsToUse.filter(function(e) {
                            return e.category === categoryKey && e.subcategory === sub.key;
                        });
                        
                        if (subEvaluations.length > 0) {
                            if (self.selectedEvaluationDate !== 'all') {
                                const sum = subEvaluations.reduce(function(acc, eval) { return acc + parseFloat(eval.score); }, 0);
                                scores.push(sum / subEvaluations.length);
                            } else {
                                subEvaluations.sort(function(a, b) { return new Date(b.evaluated_at) - new Date(a.evaluated_at); });
                ...
                createSpiderChart() {
                    ...
                    if (!canvas || !this.viewingPlayer) {
                        const self = this;
                        setTimeout(function() { self.createSpiderChart(); }, 200);
                        return;
                    }
                    ...
                    const self = this;
                    const labels = this.evaluationCategories.map(function(c) { return c.name; });
                    const data = this.evaluationCategories.map(function(c) {
                        const avg = self.getPlayerCardCategoryAverage(c.key);
                ...
                async pollForAdvice(player, attempts) {
                    ...
                    try {
                        ...
                        if (data.advice) {
                            ...
                            if (isNewAdvice) {
                                ...
                                setTimeout(function() {
                                    self.pollForAdvice(player, attempts + 1);
                                }, 5000);
                            }
                        } else {
                ...
                async downloadPlayerCardPDF(event) {
                    ...
                    try {
                        ...
                        categories.forEach(function(category, index) {
                            ...
                            const score = self.getPlayerCardCategoryAverage(category.key);
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initClubManager);
    } else {
        initClubManager();
    }
})();